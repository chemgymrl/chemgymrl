...
class Extraction:
    ...
    def reset(self, extraction_vessel):
        ...
        # generate oil vessel
        oil_vessel = vessel.Vessel(label='oil_vessel',
        ...
        oil_vessel_material_dict, _, _ = util.check_overflow(material_dict=oil_vessel_material_dict,
                                                             solute_dict={},
        ...
        return vessels, oil_vessel, util.generate_state(vessels, max_n_vessel=self.n_total_vessels)
    ...
    def perform_action(self, vessels, oil_vessel, action):
        ...
        elif 1 == int(action[0]):  # 1: Mix ExV (TBD)
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
        elif 2 == int(action[0]):  # 2: Mix B1 (TBD)
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                vessels[2].push_event_to_queue(dt=self.dt)
        elif 3 == int(action[0]):  # 3: Mix B2 (TBD)
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
        ...
        elif 4 == int(action[0]):  # 4: Add B1 to ExV (Volume multiplier) [1, 0]
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                reward = vessels[1].push_event_to_queue(events=[event], dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
        elif 5 == int(action[0]):  # 5: Add B1 to B2 (Volume multiplier) [1, 2]
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                reward = vessels[1].push_event_to_queue(events=[event], dt=self.dt)
                vessels[0].push_event_to_queue(dt=self.dt)
        elif 6 == int(action[0]):  # 6: Add ExV to B2 (Volume multiplier) [0, 2]
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                reward = vessels[0].push_event_to_queue(events=[event], dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
        elif 7 == int(action[0]):  # 7: Add Oil to ExV (Volume multiplier)
            if multiplier == 0:
                vessels[0].push_event_to_queue(dt=self.dt)
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
                ...
                vessels[1].push_event_to_queue(dt=self.dt)
                vessels[2].push_event_to_queue(dt=self.dt)
        elif 8 == int(action[0]):  # 8: Done (Value doesn't matter)
            done = True
            reward = self.done_reward(vessels[2])

        return vessels, oil_vessel, reward, done

    def done_reward(self, beaker_2):  # beaker 2 is the beaker used to collect extracted material
